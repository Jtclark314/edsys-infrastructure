...
- `deploy-netdata-compute.sh` - rollback-safe installer for the seven-node `edsys-compute` Netdata Parent/Child topology. It aligns the four Proxmox hosts and two Ubuntu satellites while keeping the runtime stream key and Cloud material outside Git.
- `verify-netdata-compute.py` - strict local-API acceptance check for the exact seven reachable nodes, shared group label, and six receiving streams.
