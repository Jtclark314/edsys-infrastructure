...
- `install-container-recovery.sh` - validated installer for the recovery manifest, systemd units, Docker drop-in, bounded shutdown timeouts, and live restore with targeted stale-socket recovery.
...
- `9950x-nvme-migration-check.sh` - read-only inventory, pre-clone, post-boot, and final-acceptance gates for the 9950x system-NVMe migration. It discovers disks dynamically, validates UUID-based boot/mount contracts, detects duplicate clone UUIDs, checks the intended two/three-disk layouts, reports PCIe link widths, and never partitions, formats, mounts, clones, repairs, or resizes a disk.
...
- `anythingllm-upload-edsys-rag` - hash-aware uploader for the sole managed AnythingLLM workspace, `EdSys-RAG-Current`. It rejects alternate workspace slugs, removes stale managed attachments, and verifies vectors after every changed upload.
- `anythingllm-rag-verify.py` - read-only integrity and retrieval gate for `EdSys-RAG-Current`. Quick mode checks the single-workspace contract, settings, manifest parity, vectors, two positive sentinels, and one unrelated negative control; the default full mode evaluates all sanitized golden queries.
- `install-anythingllm-rag-sync.sh` - user-scoped installer for the tracked uploader, verifier, RAG sync service, and five-minute timer. Replaced files are backed up beneath `~/.local/state/edsys-rag-sync/backups/`.
...
- `systemd/edsys-rag-sync.service` and `systemd/edsys-rag-sync.timer` - authoritative single-workspace refresh pipeline. The service rebuilds the clean mirror and Portal grounding index, uploads only `edsys-rag-current`, and then runs the quick retrieval gate.
